...
async function updateEnvironmentVariables(functionName, oldVars, newVars) {
    ...
    try {
        ...
        console.log(`Environment variables updated successfully for ${functionName}`);
...
async function handler() {
    ...
    if (Array.isArray(lambdas)) {
        lambdas?.forEach((lambda) => {
            ...
            if (lambda?.Environment && lambda?.Environment?.Variables && lambda?.Environment?.Variables?.ELASTICNODE) {
                updateEnvironmentVariables(lambda?.FunctionName, lambda?.Environment?.Variables, { ELASTICNODE: NEW_NODE_URL });
            }
        });
    }
}
